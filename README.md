...
decoder is 26 KB of it. This read 861 KB until the codec was split into separate
encode and decode units, which stopped the linker pulling the compressor into
decoding builds — the format was never the problem.
...
transparency and sub-8-bit non-indexed images into real G/GA/RGB/RGBA channels,
which would make those chunks invalid. `PLTE` is not carried either: an indexed
